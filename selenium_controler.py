...
def download(url, data):
	...
	mtime = time()
	try:
		mtime = path.getmtime(DOWNLOAD_DIR+"/"+(data["author"]["uniqueId"].encode("utf8").hex())+".tar")
	except:
		pass

	with tarfile.open(name=DOWNLOAD_DIR+"/"+(data["author"]["uniqueId"].encode("utf8").hex())+".tar", mode="a") as archive:
	...
	utime(DOWNLOAD_DIR+"/"+(data["author"]["uniqueId"].encode("utf8").hex())+".tar", times=(mtime,mtime))
