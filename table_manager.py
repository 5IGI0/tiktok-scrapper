...
def is_present(element_id):
	...
	if not os.path.exists(PRESENCE_DIR+"/"+table_prefix):
		return False

	with open(PRESENCE_DIR+"/"+table_prefix) as fp:
		for line in fp:
...
def add_presence(element_id, user_id):
	...
	with open(PRESENCE_DIR+"/"+table_prefix, "a") as fp:
